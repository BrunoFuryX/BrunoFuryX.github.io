...
function switchSlide(current) {
    ...
    for (let i = 0; i < $dots.length; i++) {
        $dots[i].children[0].src = "assets/images/elipse-vazio.svg";
    }

    $dots[slideIndex].children[0].src = "assets/images/elipse-cheia.svg";
    if (window.matchMedia("screen and (max-width: 360px)").matches) {
        banner.style.backgroundImage = "url('./assets/images/main-banner-p-".concat(slideIndex).concat(".jpg')");
    } else {
        banner.style.backgroundImage = "url('./assets/images/main-banner-".concat(slideIndex).concat(".jpg')");
    }
}
